...
namespace huffman_thread {
    ...
    generate_frequency_gmr(const vector<char> input, size_t n_mappers, size_t n_reducers) {
    ...
    unique_ptr<vector<bool>> encode(string &seq, unordered_map<char, vector<bool>> &codes, size_t n_encoders) {
        ...
        vector<bool> buffers[n_encoders];
        ...
        auto encode_executor = [&](size_t tid) {
            ...
            for (size_t i = start; i < end; i++) {
                auto c = seq[i];
                buffers[tid].insert(buffers[tid].end(), codes.at(c).begin(), codes.at(c).end());
            }
        };
        ...
        for (size_t i = 0; i < n_encoders; i++) {
            encoded->insert(encoded->end(), buffers[i].begin(), buffers[i].end());
        }
    ...
    void run(string &filename, size_t n_mappers, size_t n_reducers, size_t n_encoders) {
        ...
        auto seq = read_file(filename);
        vector<char> test_vector(seq.begin(), seq.end());

        /** frequency map generation **/
        cout << "> Generating frequency map (GMR)...";
        auto start = chrono::high_resolution_clock::now();
        auto freqs = generate_frequency_gmr(test_vector, n_mappers, n_reducers);
        ...
        auto encoded = encode(seq, *codes, n_encoders);

        auto end_encoded = chrono::high_resolution_clock::now();
        elapsed = chrono::duration_cast<chrono::milliseconds>(end_encoded - start_encoded).count();
        cout << "(took " << elapsed << "ms)" << endl;
